...
class ProductControllerTest(TestCase):
    ...
    def test_add_item_to_cart(self):
        ...
        url = "/api/add-item-to-cart/?user_id=" + str(self.user.id) + "&product_id=" + str(self.item.id)
        response = self.client.get(url)

        self.assertEqual(response.status_code, 201)
        self.assertEqual(Cart.objects.filter(user_id=self.user.id, item_id=self.item.id).count(), 1)
