...
class CheckoutControllerTest(TestCase):
    ...
    def test_get_stripe_public_key(self):
        with patch.object(SecretProvider, SecretProvider.get_secret.__name__, return_value="test_key"):
    ...
    def test_create_checkout_session(self):
        with(patch.object(SecretProvider, SecretProvider.get_secret.__name__, return_value="test_key"),
             patch.object(stripe.Product, stripe.Product.create.__name__, return_value=MagicMock()),
             patch.object(stripe.Price, stripe.Price.create.__name__, return_value=MagicMock()),
             patch.object(stripe.checkout.Session, stripe.checkout.Session.create.__name__,
    ...
    def test_session_status(self):
        with patch.object(stripe.checkout.Session, stripe.checkout.Session.retrieve.__name__,
                          return_value=MagicMock(status="paid", customer_details=MagicMock(email="test@test.com"))):
