...
class SecretProviderTest(TestCase):    
    ...
    def test_get_non_existing_secret_fallback(self):
        with patch('builtins.open', mock_open(read_data='[test]\ntest_key = "test_value"')):
            from fleaapi.provider.secret_provider import SecretProvider
            secret_provider = SecretProvider()
            assert secret_provider.get_secret('test', 'test_key2', 'test_value2') == 'test_value2'

    def test_get_non_existing_category_fallback(self):
        with patch('builtins.open', mock_open(read_data='[test]\ntest_key = "test_value"')):
            from fleaapi.provider.secret_provider import SecretProvider
            secret_provider = SecretProvider()
            assert secret_provider.get_secret('test2', 'test_key', 'test_value2') == 'test_value2'

    def test_get_non_existing_secret_file(self):
        with patch('builtins.open', mock_open(read_data='[test]\ntest_key = "test_value"')) as m:
    ...
    def test_get_non_toml_secret_file(self):
        with patch('builtins.open', mock_open(read_data='{"test":{"test_key":"test_value"}}')) as m:
            import toml
